...
## What's new in v0.1.1-beta.1

- **Per-section `maxTokens`** — hard cap on completion length per section, eliminates infinite continuation
- **`temperature: 0`** — deterministic extraction across all providers
- **OCR text cleaning** — strips Tesseract artefacts before LLM (`"fluency": "|"` is gone)
- **Per-section retry** — each section self-corrects independently on failure
- **Post-extraction deduplication** — removes duplicate array entries from multi-page scans
- **`sectionResults` in meta** — per-section observability (success, retries, errors)

See the full [Changelog](/docs/resume-intel/v0.1.1-beta.1/en/changelog) for details.
...
| **Observability** | `sectionResults` in meta — per-section success, retries, errors |
