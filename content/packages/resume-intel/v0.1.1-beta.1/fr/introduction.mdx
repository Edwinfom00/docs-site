...
## Nouveautés de la v0.1.1-beta.1

- **`maxTokens` par section** — limite stricte sur la longueur de complétion par section, élimine la continuation infinie
- **`temperature: 0`** — extraction déterministe sur tous les fournisseurs
- **Nettoyage du texte OCR** — supprime les artefacts Tesseract avant le LLM (`"fluency": "|"` disparaît)
- **Retry par section** — chaque section se corrige indépendamment en cas d'échec
- **Déduplication post-extraction** — supprime les entrées dupliquées dans les tableaux
- **`sectionResults` dans meta** — observabilité par section (succès, retries, erreurs)

Voir le [Changelog](/docs/resume-intel/v0.1.1-beta.1/fr/changelog) complet pour les détails.
...
| **Observabilité** | `sectionResults` dans meta — succès, retries, erreurs par section |
