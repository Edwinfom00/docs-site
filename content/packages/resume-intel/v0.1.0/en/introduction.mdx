...
Instead of one monolithic LLM call, `resume-intel` runs parallel focused extractions per section — each with its own prompt and schema. This reduces hallucinations and isolates failures.
...
| **Task Decomposition** | Parallel per-section extraction |
