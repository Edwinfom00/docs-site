...
## Nouveautés de la v0.1.2

- **15 sections** — awards, certificates, publications, volunteer, interests, references maintenant extraites (était 6)
- **Option `sections`** — choisir exactement quelles sections extraire, économise des tokens
- **Option `outputSchema`** — remplacer le schéma JSON Resume par n'importe quel schéma Zod personnalisé
- **`sectionsRequested` dans meta** — observabilité : quelles sections ont été demandées
- **CLI** — `resume-intel parse <file.pdf>` avec `--model`, `--sections`, `--output`, `--out`
- **Fix serverless** — worker chargé via `MessageChannel`, sans résolution de chemin — fonctionne sur Vercel et AWS Lambda
- **Prompts améliorés** — meilleure normalisation des dates, nettoyage des URLs, hints de déduplication

Voir le [Changelog](/docs/resume-intel/v0.1.2/fr/changelog) complet pour les détails.
