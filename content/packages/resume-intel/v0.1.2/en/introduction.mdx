---
title: Introduction
description: @edwinfom/resume-intel is a LLM-first resume parsing infrastructure — model-agnostic, spatial PDF extraction, automatic OCR fallback, JSON Resume v1 output, and production-grade JSON validation.
---

# @edwinfom/resume-intel

> LLM-first resume parsing infrastructure.
> Model-agnostic · Spatial extraction · OCR fallback · JSON Resume compatible
...
- **15 sections** — awards, certificates, publications, volunteer, interests, references now fully extracted (was 6)
- **`sections` option** — choose exactly which sections to extract, saves tokens
- **`outputSchema` option** — replace the default JSON Resume schema with any custom Zod schema
- **`sectionsRequested` in meta** — observability: which sections were requested
- **CLI** — `resume-intel parse <file.pdf>` with `--model`, `--sections`, `--output`, `--out`
- **Serverless fix** — worker loaded via `MessageChannel`, no file-path resolution — works on Vercel and AWS Lambda
- **Improved prompts** — better date normalization, URL cleaning, deduplication hints

See the full [Changelog](/docs/resume-intel/v0.1.2/en/changelog) for details.
