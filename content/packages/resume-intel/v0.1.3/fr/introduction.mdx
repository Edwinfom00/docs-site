...
## Nouveautés de la v0.1.3

- **Normaliseur de sortie** — correction du padding des dates (`"2024-01-01"` → `"2024"`), suppression des `endDate` "Présent", suppression des URLs tronquées, filtrage des profils invalides
- **Correction section work** — les projets personnels listés comme expérience professionnelle sont maintenant inclus
- **Détection de scan améliorée** — heuristique nombre de blocs + densité de caractères réduit les faux positifs
- **Option `maxConcurrency`** — limiter les appels parallèles (utile pour les APIs avec rate limiting)
- **Callback `onProgress`** — appelé après chaque extraction de section avec `(section, success)`
- **Option `ocrLanguage`** — code de langue Tesseract pour les CVs non-anglophones
- **Exports utilitaires `normalizeDate` et `cleanUrl`**

Voir le [Changelog](/docs/resume-intel/v0.1.3/fr/changelog) complet pour les détails.
