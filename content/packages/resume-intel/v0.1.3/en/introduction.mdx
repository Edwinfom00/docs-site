...
- **Output normalizer** — date padding fix (`"2024-01-01"` → `"2024"`), `"Present"` endDate removal, truncated URL removal, invalid profile filtering
- **Work section fix** — personal projects listed as work experience are now included
- **Improved scan detection** — block count + char density heuristic reduces false positives
- **`maxConcurrency` option** — limit parallel section calls (useful for rate-limited APIs)
- **`onProgress` callback** — called after each section extraction with `(section, success)`
- **`ocrLanguage` option** — Tesseract language code for non-English CVs
- **`normalizeDate` and `cleanUrl` utility exports**

See the full [Changelog](/docs/resume-intel/v0.1.3/en/changelog) for details.
